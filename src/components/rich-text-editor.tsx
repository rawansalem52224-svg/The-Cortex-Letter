  ...
  return (
    <button
      ...
      className={`inline-flex h-8 w-8 items-center justify-center rounded-md border transition-colors ${
        active
          ? "border-[var(--accent-teal,#0f766e)] bg-[var(--accent-teal,#0f766e)]/10 text-[var(--accent-teal,#0f766e)]"
          : "border-transparent text-[var(--ink,#1a1a2e)]/70 hover:bg-[var(--ink,#1a1a2e)]/5"
      }`}
    >
      {children}
    </button>
...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-neutral max-w-none min-h-[300px] px-4 py-3 focus:outline-none font-[var(--font-body,inherit)]",
      },
    },
    onUpdate: ({ editor }) => {
  ...
  return (
    <div className="rounded-lg border border-[var(--ink,#1a1a2e)]/15 bg-[var(--paper,#fff)]">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-1 border-b border-[var(--ink,#1a1a2e)]/10 p-2">
        <ToolbarButton
          label="Bold (Ctrl+B)"
        ...
        <div className="mx-1 h-5 w-px bg-[var(--ink,#1a1a2e)]/10" />
